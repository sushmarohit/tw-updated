...
const TOOL_SLUG = 'fundraise/runway-check';

export default function RunwayCheckPage() {
  const { t } = useTranslation(['tools', 'common']);
  const [step, setStep] = useState<'form' | 'result'>('form');
  const [formData, setFormData] = useState<RunwayInput>({ cashBalance: 0, monthlyBurn: 0 });
  ...
  const handleCalculate = () => {
    trackCalculatorStart('fundraise-runway');
    const res = calculateRunway(formData);
    setResult(res);
    setStep('result');
  };
  ...
  const handleSubmit = async () => {
    ...
    trackCalculatorComplete('fundraise-runway', result.runwayMonths);
    try {
      ...
      if (data.success) {
        ...
        trackToolCompletedWithEmail(TOOL_SLUG, result.runwayMonths, userInfo.email || undefined);
      }
    } catch {
      // ignore
    } finally {
    ...
    return (
      <div className="min-h-screen bg-gray-50 section-padding">
        <div className="container-custom max-w-2xl">
          ...
          <div className="card">
            <h1 className="heading-h2 mb-4">{t('tools:fundraise.runwayCheck.name')}</h1>
            <div className="p-6 bg-teal-50 rounded-lg mb-6">
              <p className="body-small text-gray-600 mb-1">Runway</p>
              <p className="text-4xl font-bold text-teal-600">{result.runwayMonths} months</p>
              <p className="body-small text-gray-600 mt-2">Status: {result.runwayLabel}</p>
            </div>
            {showUserInfoForm && (
            ...
            <div className="flex gap-4">
              {showUserInfoForm && (
                <Button variant="primary" onClick={handleSubmit} disabled={isSubmitting}>{isSubmitting ? t('common:loading') : t('common:saveResults')}</Button>
              )}
      ...
      <div className="container-custom max-w-2xl">
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            <div>
              <label className="block font-semibold mb-2">Cash balance (₹)</label>
              <input type="number" className="input" value={formData.cashBalance || ''} onChange={(e) => setFormData({ ...formData, cashBalance: parseFloat(e.target.value) || 0 })} placeholder="5000000" />
            </div>
            <div>
              <label className="block font-semibold mb-2">Monthly burn (₹)</label>
              <input type="number" className="input" value={formData.monthlyBurn || ''} onChange={(e) => setFormData({ ...formData, monthlyBurn: parseFloat(e.target.value) || 0 })} placeholder="500000" />
            </div>
            <Button variant="primary" className="w-full" onClick={handleCalculate} disabled={!formData.cashBalance || !formData.monthlyBurn}>
              {t('common:calculate')}
            </Button>
          </div>
        </div>
